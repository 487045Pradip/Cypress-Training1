...
describe("API Tests", () => {
    ...
    it.only("API TC1: Get All Products List", () => {
        cy.request({
            method: "GET",
            ...
            expect(parsedBody.products.filter(product=>{
                product.category.usertype.usertype==='Women'
            }).some(product=>product.category.category=='Tshirts')).to.be.false
            //console.log (`Product list is: ${wrap.parsedBody}`)
        })
    })
